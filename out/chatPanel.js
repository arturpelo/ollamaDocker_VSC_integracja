...
class ChatPanel {
    constructor(panel, client, model, temperature) {
        this.client = client;
        this.model = model;
    ...
    static createOrShow(client, model, temperature) {
        ...
        const panel = vscode.window.createWebviewPanel("ollamaChat", "Ollama Chat", column ?? vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
    ...
    async _handleMessage(msg) {
        if (msg.type === "send" && msg.text) {
            ...
            try {
                let full = "";
                await this.client.chat(this._history, this.model, this.temperature, (token) => {
                    full += token;
